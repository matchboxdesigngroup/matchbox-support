...
class Userback {
	...
	public function initialize_userback() {
		...
		if ( ! empty( $token ) && $token_valid ) {
			$is_authorized_user = current_user_can( 'manage_options' ) || current_user_can( 'edit_posts' );
			$show_public        = get_option( 'matchbox_userback_public' );
			...
			} elseif ( $show_public && ! is_user_logged_in() ) {
				add_action( 'wp_footer', array( $this, 'print_script' ), 100 );
			}
		}
